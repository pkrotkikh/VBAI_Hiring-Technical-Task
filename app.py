...
class SpeechHandler:
    ...
    def stop_recording(self):
        ...
        if self.recording_thread:
            self.recording_thread.join(timeout=0.5)  # Increased timeout for completion
        self.recording = False
        if hasattr(self, 'last_frames') and self.last_frames:
            print(f"[SpeechHandler] Processing {len(self.last_frames)} frames of recorded audio")
    ...
    def _record_audio(self):
        ...
        try:
            while not self.stop_recording_event.is_set():
                ...
                frames.append(data)
                print(f"[SpeechHandler] Recorded chunk, total frames: {len(frames)}")
            # Minimum recording duration is 2 seconds
            while time.time() - start_time < 2.0 and not self.stop_recording_event.is_set():
                data = stream.read(self.chunk, exception_on_overflow=False)
                frames.append(data)
                print(f"[SpeechHandler] Recorded extra chunk, total frames: {len(frames)}")
    ...
    async def _process_speech_async(self, audio_data):
        try:
            ...
            print(f"[SpeechHandler] Transcription result: {transcript}")
            return transcript
        except Exception as e:
    ...
    def _process_speech(self, audio_data):
        ...
        transcript = loop.run_until_complete(self._process_speech_async(audio_data))
        loop.close()
        if transcript:
            print(f"[SpeechHandler] Adding to queue: {transcript}")
            self.audio_queue.put(transcript)
            self.generate_speech(transcript, voice="echo", speed=0.9)
        else:
            print("[SpeechHandler] No transcript generated")

    async def _generate_speech_async(self, text, voice="alloy", speed=0.9):
        try:
            ...
            wav_buffer = io.BytesIO()
            audio_segment.export(wav_buffer, format="wav", parameters=["-ar", "48000", "-ac", "1", "-sample_fmt", "s16"])
            wav_data = wav_buffer.getvalue()
            print(f"[SpeechHandler] Generated audio for: {text}")
            return wav_data
        except Exception as e:
            print(f"[SpeechHandler] Speech generation error: {e}")
            return None

    def generate_speech(self, text, voice="alloy", speed=0.9):
    ...
    def _playback_queue(self):
        while not self.stop_playing.is_set():
            try:
                text, voice, speed = self.speech_playback_queue.get(timeout=0.1)
    ...
    def get_transcript(self, timeout=2.0):
        try:
...
class DialogueSystem:
    def __init__(self):
        ...
        self.npc_voices = {
            "HR": {"voice": "nova", "speed": 0.9, "personality": "warm"},
            "CEO": {"voice": "onyx", "speed": 0.9, "personality": "confident"}
        }
    ...
    def start_conversation(self, npc_role="HR", player_pos=None):
        ...
        # Voice the initial message of the NPC even without active speech mode
        npc_voice_config = self.npc_voices.get(npc_role, {"voice": "alloy", "speed": 0.9})
    ...
    def send_message(self):
        ...
        try:
            ...
            npc_voice_config = self.npc_voices.get(self.current_npc, {"voice": "alloy", "speed": 0.9})
            self.speech_handler.generate_speech(
                ai_message,
                voice=npc_voice_config["voice"],
                speed=npc_voice_config["speed"]
    ...
    def render(self):
        ...
        if self.use_speech and self.input_active:
            transcript = self.speech_handler.get_transcript(timeout=2.0)
            if transcript:
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_RETURN:
                ...
                if self.user_input.strip():
                    ...
                    self.speech_handler.generate_speech(self.user_input, voice="echo", speed=0.9)
                    self.user_input = ""
                    self.send_message()
